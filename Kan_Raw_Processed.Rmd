...
Because of a mistake during the creation of the first experiment we did not log the last word of each sentence. However, the program logs the participants answer in the following line in the variable of the specific modul [keyboard_input_3]. However, opensesame iterates the last pressed answer option in the variable, therefore we kept only those values that are following the missing trial.
...
Excluding the paticipants and creating a new dataset for this analysis.
...
Each participant should have complated XXX trials () during the test part of the experiment.
...
After cleaning the dataset and addding the missing information about the sentences, we create a dataset for the main accuracy and rt analysis of the first experiement.
...
Each sentence is diveded into sentence regions. A sentence region can be consisted of multiple words. This file can be found in the supplementary materials and it contains all the information about the sentence regions.
...
When we are calculating the number of characters in each word.
...
In the Kan et al. (2013) paper each sentence region had only one data point from each participant. Therefore, we avarage the residual reading times of the sentences for each sentence region per congruency condition per participant.
...
Now we fit the modell to the inconguent sentences to calculate the predicted reading time.
...
We expect to find a difference in the temporarily ambiguous region and the disambiguating region. The sentence region in the incongruent sentences containing the temporarly ambiguous words is the 3rd region, while the disambiguation region is the 4th region.
...
Checking the number of trials per parciticpant. Each participant should have 271 trials:
...
We create a varaible that mark which lab collected the data (Australia, Hungary, Singapore).
...
There are missing values in the loopnum variable because of the way opensesame saves variables. Therefore, we fill up the missing values for each participant.
...
We did not record the exact responses for the incongruent necker trials but the number of reversals in the variable [reversal]. We add the content of this varible to the [response] variable.
...
The number of trials that we will include in the analysis are different for each set of trials. As we did not know the original order of the trials for the experiment we created 5 sets of trials for Exp2 and randomly assigned each participant to a set of trials. To which set a participant was assigned to is saved in the [loop_num] variable.
...
main_analysis_data %>% 
  filter(is_previous_necker == 1L) %>% 
  group_by(loop_num, conflict_condition) %>% 
  count() %>% 
  group_by(loop_num) %>% 
  mutate(N = sum(n),
         prop = round(n / N * 100, 2)) %>% View()
```
...
Check the avarge number of experienced reversals across participants:
...
Join the calculated reversal group assignment with the full dataset for the accuracy analysis.
...
The number of conditions per participant. Because we excluded the incorrect trials, it can happen that a participant does not have reaction time responses from all the 4 conditions.
...
```{r}
write_tsv(main_analysis_rt_conf_data, "Data/Processed/Exp2/Kan_Processed_Exp2_Main_Rt_Conf_data.tsv")
```
...
```{r}
raw <- read_tsv("Data/Raw/Exp3/Kan_Raw_Exp3_data.tsv")
```
