...
### Excluding participants because of the comprehension probe
...
As we first excluded those participants who had missing trials we exclude those first here as well so that the number of people who was excluded because of the comprehension probe test will not overlap with the former exclusion.
...
Descriptive statistics of those participants who scored above 70% on the sentence comprehension task.
...
Number of people scoring above 70% level.
...
```{r}
sentence_comprehension %>%  
  filter(correct == 1 & freq > 50) %>% 
  summarise(mean_comprehension_score = mean(comprehension_score),
            sd_comprehension_score = sd(comprehension_score))
```

Number of people scoring above 50% level.
...
Number of people who we exclude because of comprehension probe.

```{r}
count(exclude_comprehension)
```
...
```{r}
write_tsv(main_analysis_data, "Data/Processed/Exp1/Kan_Processed_Exp1_Main_Acc_data.tsv")
```

## Outcome neutral tests data preprocessing
### Sentence task

We exclude the participants from this analysis because of the comprehension probe, and because of missing conditions.

```{r}
neutral_analysis_sentence <-
  anti_join(test_raw, exclude_comprehension, by = "participant_id") %>% 
  anti_join(., missing_condition, by = "participant_id")
```
...
Now that we have the fitted model in the model variable for each sentence region, we calculate the predicted reading times of the model for the congruent sentences.
...
One of the reviewers suggested to use both the incongruent and the congruent sentences to calculate the residual reading times with thee omission of the filler sentences. We decided to include the result of this calculation in the supplementary materials as a sensitivity test. The decisions will be made on the registered method presented earlier.

#### Calculating the residual reading times
 
In order to account for the biasing effect of the length of the sentence region on the reading time, we calculated the residual reading time for each sentence region. For every sentence-region (see the supplementary materials for the partition of each sentence into sentence-regions) of each participant we applied a simple linear regression with the actual reading time as the outcome variable and the length of the region in the number of characters as the predictor variable.

At the sensitivity test we included only the congruent (N = 21) and the incongruent (N = 21) in the linear regression.

```{r}
sensitivity_residual_model <-
  mean_sentence_values %>% 
  filter(sentence_type != "Filler") %>%
  group_by(participant_id, sentence_region_no) %>% 
  group_nest() %>% 
  mutate(model = map(data,
                     ~ lm(mean_sentence_region_reading_time ~ mean_sentence_region_word_length,
...
We run a few checks of the models.
...
mean_sentence_values %>% 
  ...
  summarize(number_of_sentences = n()) %>% 
  ungroup() %>% 
  mutate(number_of_all_sentences = sum(number_of_sentences))
```

There are 71 sentences but after the exclusion of the 29 filler sentences there are 42 sentences left. The maximum number of sentence regions is 8. As we have 132 participants we will have 132 * 8 = 1056 data points.

__NOTE:__ Even tho sentence 57 is a congruent sentence which originally has 9 sentence regions, after excluding the third sentence region at this part of the analysis it only has 8.
...
Now that we have the fitted model in the model variable for each sentence region, we calculate the predicted reading times of the model for the congruent sentences. This function also calculates the residual reading times because the data provided for the model is the same as the data that will be augmented with .fitted and .resid.

```{r}
sensitivity_residual_model <-
  sensitivity_residual_model %>% 
  mutate(residual_reading_time = map2(data, model,
                                           ~ broom::augment(x = .y,
                                                            data = .x)))
```
...
```{r}
sensitivity_residual_model_aggregated <- 
  sensitivity_residual_model %>% 
  select(participant_id, sentence_region_no, residual_reading_time) %>% 
  unnest(cols = c(residual_reading_time)) %>% 
  rename(residual_reading_time = .resid) %>% 
  group_by(participant_id, sentence_type, sentence_region_no) %>%
  summarise(mean_residual_reading_time = mean(residual_reading_time))
```
...
```{r}
write_tsv(sensitivity_analysis_sentence, "Data/Processed/Exp1/Kan_Processed_Exp1_Sensitivity_Sentence_data.tsv")
```
...
The number and proportion of test trials per conflict condition per set of trials.
...
Excluding those participants have 0% accuracy.
...
all_incorrect <- 
  ...
  filter(is_previous_necker == 1L) %>% 
  group_by(participant_id) %>% 
  filter(all(is_correct == 0)) %>% 
  ungroup() %>% 
  distinct(participant_id)
```

Number of participants who have 0% accuracy.
...
main_analysis_data <-
  main_analysis_data %>%
  anti_join(., all_incorrect, by = "participant_id")
```
...
Deleting not needed variables before saving.

```{r}
main_analysis_rt_conf_data <- 
  main_analysis_rt_conf_data %>% 
  select(-reversal, -is_previous_necker, -procedure)
```
...
The number of participants left after this exclusion.
...
The number of participants who we excluded because of the comprehension probe.

```{r}
comprehension_probe_data %>% 
  count(drop_comprehension)
```
...
Drop not needed variables before saving.

```{r}
main_analysis_rt_data <-
  main_analysis_rt_data %>%
  select(-procedure, -reversal, -score, -is_previous_necker, -drop_comprehension)
```
