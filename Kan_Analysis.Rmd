...
outcome_neutral_sentence_data <-
  ...
  mutate(t_value = map_dbl(data,
                       ~ t.test(.$Incongruent, .$Congruent, paired = TRUE, alternative = "less") %>% 
                         broom::tidy() %>% 
                         pull(statistic)),
         df = map_dbl(data,
                       ~ t.test(.$Incongruent, .$Congruent, paired = TRUE, alternative = "less") %>% 
...
#TODO: this should be equal for all regions.
...
sentence_region_plot <- function(df, name) {
  ...
  geom_errorbar(aes(ymin = mean - (se * 1.96),
                    ymax = mean + (se  * 1.96)),
                width = .1) +
  labs(title = paste("Sentence region no:", name),
...
anova_rt_f_value <-
  ...
  transmute(term = case_when(term == "is_congruent" ~ "main_effect_congruency",
                             term == "is_previous_congruent" ~ "main_effect_previous_congruency",
...
main_analysis_rt_raw_effect <-
  main_analysis_rt_raw_effect %>% 
  summarise(main_effect_congruency = mean(congruency_effect, na.rm = T),
            main_effect_previous_congruency = mean(previous_congruency_effect, na.rm = T),
...
main_se <- 
  main_analysis_rt_raw_effect %>% 
  filter(term == "main_effect_congruency") %>% 
  pull(se)
...
anova_acc_f_value <-
  ...
  transmute(term = case_when(Effect == "is_congruent" ~ "main_effect_congruency",
                             Effect == "is_previous_congruent" ~ "main_effect_previous_congruency",
